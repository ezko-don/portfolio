      ...
      <div className="md:max-w-fit md:border-2 md:rounded-full mx-auto px-7 py-2 bg-zinc-200/50 dark:bg-slate-900/50 backdrop-blur-3xl">
        <div className="flex justify-between items-center gap-10">
          <motion.div
            ...
            className="text-xl font-bold bg-gradient-to-r from-pink-600 via-purple-600 to-pink-600 dark:from-pink-400 dark:via-purple-400 dark:to-pink-400 bg-clip-text text-transparent"
          >
            EZ
          </motion.div>
          <div className="flex items-center space-x-8">
            <div className="hidden md:flex items-center space-x-8">
              {["Overview", "Stack", "Experience", "Media", "Projects", "Testimonials", "Certifications", "Contact"].map((item, index) => (
                <motion.a
                  key={item}
            ...
            >
              {theme === "dark" ? (
                <Icon icon="solar:sun-bold" className="text-pink-500" width={20} height={20} />
              ) : (
                <Icon icon="solar:moon-bold" className="text-purple-500" width={20} height={20} />
              )}
            </motion.button>
          </div>
        </div>
