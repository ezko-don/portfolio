  ...
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden">
      ...
      {Array.from({ length: particleCount }).map((_, i) => {
        ...
        const left = ((i * 8.33) % 100).toFixed(2); // 100/12 ≈ 8.33 for even distribution
        const top = ((i * 23.5) % 100).toFixed(2); // Using prime number for better distribution
        ...
        return (
          <motion.div
            ...
            transition={{
              duration: 4 + (i % 2), // Deterministic duration based on index
              repeat: Infinity,
              delay: i * 0.25, // Deterministic delay based on index
              ease: "linear",
            }}
          />
        );
